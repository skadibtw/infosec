...
Dependency-Check нашёл в PyJWT 2.14.0 уязвимость **CVE-2025-45770** (CVSS 7.0, «weak encryption»): библиотека не ограничивает минимальную длину HMAC-ключа, поэтому с коротким ключом токены можно подделать перебором. CVE оспорена: длину ключа должно задавать приложение, а не библиотека. Исправленной версии PyJWT нет, поэтому проблема устранена на стороне приложения: `app.py` отказывается запускаться с `JWT_SECRET` короче 32 символов, а по умолчанию генерирует случайный 256-битный ключ. После этого CVE подавлена в `dependency-check-suppressions.xml` с обоснованием, и файл передаётся сканеру через `--suppression`.
