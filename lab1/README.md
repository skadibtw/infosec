...
| `JWT_SECRET` | ключ подписи JWT, не короче 32 символов (иначе приложение не запустится). Если не задан, генерируется случайно при каждом запуске |
...
**JWT.** После успешного входа сервер выдаёт токен, подписанный HMAC-SHA256 секретным ключом `JWT_SECRET`. Ключ короче 32 символов приложение не принимает и при старте падает с ошибкой, чтобы токены нельзя было подделать перебором слабого ключа. В payload лежат `sub` (имя пользователя) и `exp` (время истечения, +1 час).
...
Dependency-Check пометил PyJWT 2.14.0 уязвимостью **CVE-2025-45770** (CVSS 7.0, «weak encryption», отсутствие ограничения на минимальную длину ключа). Разбор показал, что это ложное срабатывание: по данным NVD уязвимость относится к PHP-библиотеке [lcobucci/jwt](https://github.com/lcobucci/jwt) (CPE `jwt_project:jwt` ≤ 5.4.3), а с PyJWT её связал эвристический CPE-анализатор только по совпадению слова «jwt» в названии. PyJWT 2.14.0 (последняя версия) в список затронутых не входит, поэтому обновлять нечего.

Срабатывание подавлено в `dependency-check-suppressions.xml` с обоснованием, и файл передаётся сканеру через `--suppression`. Для защиты от самой описанной угрозы (подбор слабого HMAC-ключа) приложение всё равно не запускается с `JWT_SECRET` короче 32 символов.
